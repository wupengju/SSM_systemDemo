...
public class JWTCheckInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws IOException, TokenExpiredException {
        ...
        if (StringUtils.isEmpty(jwt) || StringUtils.isEmpty(currentName)) {
            return buildResponseResult(response, ErrorStateEnum.ILLEGEL_OPERATION);
        }

        // Token 超时
        PayloadInfo payloadInfo = JWTUtil.decrypt(jwt, PayloadInfo.class);
        logger.info("decrypt jwt after:{}", JSON.toJSONString(payloadInfo));
        if (payloadInfo == null) {
            return buildResponseResult(response, ErrorStateEnum.TOKEN_TIMEOUT);
        }

        // 验证当前登录用户和Token是否匹配
        return currentName.equals(payloadInfo.getUsername()) || buildResponseResult(response, ErrorStateEnum.TOKEN_VALIDATE_FAIL);
    }
